...
function validateCreateRequest(req,res,next){
    ...
    if(!compareTime(req.body.arrivalTime,req.body.departureTime)){
        ErrorResponse.message='Something went wrong while creating an flight';
        ErrorResponse.error=new AppError(['departureTime and arrivalTime not found in the incoming request in the correct form'],StatusCodes.BAD_REQUEST);
        return res
              .status(StatusCodes.BAD_REQUEST)
              .json(ErrorResponse);     
    }
...
function validateUpdateSeatsRequest(req,res,next){
    if(!req.body.seats){
        ...
        ErrorResponse.error= new AppError(['seats not found in the incoming request in the form '],StatusCodes.BAD_REQUEST);
        return res
                  .status(StatusCodes.BAD_REQUEST)
                  .json(ErrorResponse)
    }
